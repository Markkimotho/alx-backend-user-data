#!/usr/bin/env python3
"""This module contains test scenarios for user authentication functionality.
"""
import requests
...
def log_in_wrong_password(email: str, password: str) -> None:
    ...
    assert response.status_code == 401, "Incorrect password login should return 401"
...
def profile_logged(session_id: str) -> None:
    ...
    assert response.status_code == 200, "Failed to get profile"
...
def reset_password_token(email: str) -> str:
    ...
    data = {"email": email}
    response = requests.post(url, json=data)
    assert response.status_code == 200, "Failed to request reset password token"
